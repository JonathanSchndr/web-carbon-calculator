...
export async function analyzeWebsite(url: string, origin?: string): Promise<AnalysisResult> {
  let domain = ''
  try {
  ...
  const htmlRes = await fetch(url.startsWith('http') ? url : `https://${url}`, {
    headers: {
      'User-Agent': `Mozilla/5.0 (compatible; WebCarbonCalculator/1.0; +${origin || 'https://web-carbon-calculator.vercel.app'})`
    }
  })

  if (!htmlRes.ok) {
    throw new Error('Unable to access this website. It might be down or blocking automated requests.')
  }
